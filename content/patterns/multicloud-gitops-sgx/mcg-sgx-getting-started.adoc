...
Install Gramine Shielded Containers prerequisites according to [GSC documentation](https://gramine.readthedocs.io/projects/gsc/en/latest/#prerequisites).
...
Create manifest file which contains conversion options, sample below:
+
[source,terminal]
----
cat << EOF > vault.manifest
sgx.enclave_size = "2G"
#sgx.max_threads = 16
sgx.max_threads = 512
...
fs.insecure__keys.default = "ffeeddccbbaa99887766554433221100"                                         <---- WHAT ABOUT IT???? 
...
---
where `<HTTPS_PROXY>`, `<HTTP_PROXY>` and `<NO_PROXY>` has to be set appropriately according to your organization proxy settings. 
...
docker tag gsc-vault-without-sgx seahorse910/sgx-gramine:rh-vault-no-docker-entrypoint-sgx-gramine

...
Gramine Shielded Containers application requires a machine with the same OS as input converted image in order to create SGX convered image. In this case it would be Red Hat Enterprise Linux 8.8. +
This machine doesn't have to have SGX feature enabled becuase it is used for conversion purposes only.

.Procedure

. Create modified Vault docker file which will be input for SGX conversion process: +
First, create new directorory and change it:
+
[source,terminal]
----
$ mkdir vault
...
$ cd vault
----
Create the `Dockerfile` with below content:
+
[source,dockerfile]
----
...
. Create `vault.manifest`  file which contains conversion configuration:
+
[source,ini]
----
sgx.enclave_size = "2G"
...
. Build secure image using `gsc` command (use when not behind corporate proxy):
+
[source,terminal]
----
$ ./gsc build --insecure-args vault-without-sgx vault.manifest
----

. (Optional) If machine, which is used for conversion process is behind corporate proxy, then use this command instead of above one:
+
[source,bash]
----
$ ./gsc build --insecure-args \
    ...
    --build-arg NO_PROXY=<NO_PROXY> \
    vault-without-sgx vault.manifest
----
`<HTTPS_PROXY>`, `<HTTP_PROXY>` and `<NO_PROXY>` has to be set appropriately according to your organization proxy settings.
...
. (Optional) Push secured container image to registry which will be available to OpenShift:
If needed, login to image registry server:
+
[source,terminal]
----
$ docker login -u=<USER_NAME> docker.io
----
`<USER_NAME>` is login of image registry server
Tag secured container image with appropriate tag and push the image to a registry:
+
[source,terminal]
----
...
----
`<DESTINATION_IMAGE_NAME` is name of the image in a registry
...
[source,terminal]
----
oc -n multicloud-gitops-intel-hub get route hub-gitops-server -ojsonpath='{.spec.host}'
----

...
* {intel-sgx} enabled in BIOS (Basic Input Output System) on workers machines - specific steps depends on platform.
...
* Machine with the Ubuntu 22.4 (for conversion purposes - doesn't need to have SGX feature enabled) and git.
...
. Prepare and build `hello-world` docker image and convert it to secure version using GSC - described below.
...
== Prepare docker image and convert it to use Intel SGX
...
. Create new file _hello-world.manifest_ containing parameters of conversion:
+
[source,file]
----
...
----
. Push secured container image to registry which is available on the cluster:  
...
. Fill the *Name* of the pattern `multicloud-gitops-sgx-hello-world` and *Cluster Group Name* `hub` (from _values-global.yaml_ file).
